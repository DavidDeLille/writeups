...
unsigned int my_f(unsigned int x, unsigned int i, unsigned int j, unsigned int k, unsigned int l){
	unsigned int t = i*x*x*x + j*x*x + k*x +l;
	return e[t%1000](x) ^ (t%251);
}

int test(unsigned int i, unsigned int j, unsigned int k, unsigned int l){
	if (my_f(i, i, j, k, l) != 8)
		return 0;

	if (my_f(j, i, j, k, l) != 223)
		return 0;

	if (my_f(k, i, j, k, l) != 137)
		return 0;
...
int main()
{
	...
	for (i = 32; i < 128; i++){
		printf("i = %d\n", i);
	for (j = 32; j < 128; j++){	if ( j == i) continue;
	for (k = 32; k < 128; k++){	if ( k==i || k== j ) continue;
	for (l = 32; l < 128; l++){	if ( l==i || l== j || l ==k ) continue;
		if (test(i,j,k,l) != 0){
			puts("\n");
			printf("%d, %d, %d, %d\n", i, j, k, l);
			printf("%c, %c, %c, %c\n", i, j, k, l);
